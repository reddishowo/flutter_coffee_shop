    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            children: [
              // 1. PROFILE HEADER (Dengan Logika Foto Lokal)
              Obx(() {
                final user = auth.userData;
                // Ambil path lokal dari AuthService
                String localPath = auth.localProfilePath.value;
                
                ImageProvider imageProvider;
                if (localPath.isNotEmpty && File(localPath).existsSync()) {
                   // Prioritas 1: Gambar Lokal (Gallery)
                ...
                return Column(
                  children: [
                    CircleAvatar(
                      ...
                      backgroundImage: imageProvider, // Gambar dinamis
                    ),
              ...
              // 2. WEATHER WIDGET (DAPAT DIKLIK -> KE DETAIL)
              GestureDetector(
                onTap: () {
                   // Cek agar tidak klik saat loading
                   if (!controller.isWeatherLoading.value && !controller.isWeatherError.value) {
                      Get.toNamed(Routes.WEATHER_DETAIL);
                   }
                },
                child: Container(
                  ...
                  child: Obx(() {
                    ...
                    // C. Success State (Menggunakan Getter dari Controller baru)
                    // Pastikan AccountController sudah diupdate sesuai langkah sebelumnya (menggunakan .temp, .weatherCode, dll)
                    final temp = controller.temp; 
                    final isHot = temp > 25;
                        ...
                        return Column(
                          children: [
                            ListTile(
                              ...
                              subtitle: const Text("Kelola Menu Produk", style: TextStyle(fontSize: 12)),
                              trailing: const Icon(Icons.arrow_forward_ios, size: 16, color: Colors.deepOrange),
                              onTap: () => Get.toNamed(Routes.ADMIN_DASHBOARD), // Ke Dashboard
                            ),
